 ...
 */
#include "keyboard.h"
#include "keylog.h"   /* instrumentation: 記錄 KeyDown 佇列事件 */
	...
	while (SDL_PollEvent(&ev))
	{
		switch (ev.type)
		{
			...
			case SDL_KEYDOWN:
				...
				/* instrumentation: 記錄此 KeyDown（含 repeat）供重播選單導航 */
				keylog_keydown(ev.key.keysym.sym, ev.key.keysym.scancode, ev.key.keysym.mod);
